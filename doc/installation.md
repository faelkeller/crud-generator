...
To get started, you should add the `faelkeller/crud-generator` Composer dependency to your project:
```
composer require faelkeller/crud-generator --dev
```
Once the package is installed, you should register the `Faelkeller\CrudGenerator\CrudGeneratorServiceProvider` service provider. Normally, Laravel 5.5+ will register the service provider automatically.
...
public function register()
{
    if ($this->app->environment() == 'local') {
        $this->app->register('Faelkeller\CrudGenerator\CrudGeneratorServiceProvider');
    }
}
```
